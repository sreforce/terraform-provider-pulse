...
func TestComponentResourceCreateUsesExternalKeyAndRemoteIdentity(t *testing.T) {
	...
	remote := client.Component{
		...
		RelevanceTagIDs: []string{"tag-production", "tag-example"},
	...
	if got, want := requests[0].RelevanceTagIDs, []string{"tag-example", "tag-production"}; !reflect.DeepEqual(got, want) {
...
func TestComponentResourceUpdateUsesPriorRevisionAndFullConfiguration(t *testing.T) {
	...
	planned := current
	planned.Name = types.StringValue("Example service alerts")
	...
	if gotRequest.Name != "Example service alerts" || gotRequest.OwnerTeamID != nil || !gotRequest.AlertEnabled {
