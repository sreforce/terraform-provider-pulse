...
func TestCurrentOrganizationDataSourceReadsCredentialOrganization(t *testing.T) {
	...
	configuredClient := &fakeDataSourceClient{
		organization: client.Organization{ID: "org-1", Name: "Example Organization", Slug: "example-org"},
	}
	...
	if got, want := state.Name.ValueString(), "Example Organization"; got != want {
		t.Fatalf("name = %q, want %q", got, want)
	}
	if got, want := state.Slug.ValueString(), "example-org"; got != want {
...
func TestComponentDataSourceReadsByUUIDAndPreservesComputedIdentity(t *testing.T) {
	...
	if got, want := state.ExternalKey.ValueString(), "production/platform/example-service"; got != want {
		t.Fatalf("external key = %q, want %q", got, want)
	}
	if got, want := state.Name.ValueString(), "Example service"; got != want {
		t.Fatalf("name = %q, want %q", got, want)
	}
