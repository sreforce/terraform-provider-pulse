...
func TestServerMatchesRequestsWithoutRecordingAuthorization(t *testing.T) {
	...
	mock := NewServer(t, "test-token", Expectation{
		...
		IfMatch:        "7",
		RequestBody:    []byte(`{"name":"Example service"}`),
		StatusCode:     http.StatusOK,
		ResponseBody:   Fixture(t, "component.json"),
	})

	request, err := http.NewRequest(http.MethodPatch, mock.URL()+"/api/automation/v1/components/component-id", bytes.NewReader([]byte(`{"name":"Example service"}`)))
	if err != nil {
