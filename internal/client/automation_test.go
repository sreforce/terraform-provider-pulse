...
func TestCreateComponentUsesCanonicalContract(t *testing.T) {
	...
	if result.ID != testComponentID || result.ExternalKey != "production/platform/example-service" || result.State != ComponentStateUnknown {
...
func TestComponentUpdateAndArchiveUseFreshKeysAndQuotedRevision(t *testing.T) {
	...
	updateBody := []byte("{\"name\":\"Example service\",\"component_type_id\":\"00000000-0000-4000-8000-000000000201\",\"owner_team_id\":null,\"relevance_tag_ids\":[],\"filter_tag_ids\":[],\"alert_enabled\":false}\n")
...
func TestCatalogPathsAndDirectEnvelopes(t *testing.T) {
	...
	if err != nil || organization.Slug != "example-org" {
